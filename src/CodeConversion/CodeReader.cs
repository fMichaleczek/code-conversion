...
namespace CodeConversion
{
    ...
    internal class Visitor : CSharpSyntaxVisitor
    {
        ...
        public Node VisitSyntaxNode(CSharpSyntaxNode node)
        {
            ...
            // System.Console.WriteLine($"{_currentNode.GetType().Name} :\n------------------------------------------------------------\n{originalSource}\n");
        ...
        public override void VisitAttribute(AttributeSyntax node)
        {
            ...
            {
                var argumentNode = VisitSyntaxNode(argument) as AttributeArgument;
                arguments.Add(argumentNode);
            }

            _currentNode = new Attribute(node.Name.ToString(), arguments);
        }
        ...
        public override void VisitAttributeArgument(AttributeArgumentSyntax node)
        {
            ...
            // System.Console.WriteLine($"Kind: {kind}");
            
            if (kind == SyntaxKind.NameColon)
            {
                expression = VisitSyntaxNode(node.NameColon);
            }
            else if (kind == SyntaxKind.NameEquals)
            {
                expression = VisitSyntaxNode(node.NameEquals);

            }
            else if (kind == SyntaxKind.SimpleMemberAccessExpression)
            {
                // Type.Member
                expression = VisitSyntaxNode(node.Expression);
            }
            else 
            {
            ...
            _currentNode = new AttributeArgument(expression);
        ...
        public override void VisitImplicitObjectCreationExpression(ImplicitObjectCreationExpressionSyntax node)
        {
            ...
            // _currentNode = new ImplicitObjectCreation(expression, argumentList);
        ...
        public override void VisitInterpolatedStringExpression(InterpolatedStringExpressionSyntax node)
        {
            ...
            // node.StringEndToken
            // _currentNode = new InterpolatedStringExpression()
        }
        ...
        public override void VisitSimpleLambdaExpression(SimpleLambdaExpressionSyntax node)
        {
            ...
            // node.Parameter
            // node.Modifiers
            // _currentNode = new SimpleLambdaExpression()
        ...
        public override void VisitConditionalAccessExpression(ConditionalAccessExpressionSyntax node)
        {
            ...
            // node.GetLocation();
            // node.WhenNotNull;
            // _currentNode = new ConditionalAccessExpression()
        }
        ...
        public override void VisitFileScopedNamespaceDeclaration(FileScopedNamespaceDeclarationSyntax node)
        {
            /*
                 ...
                 Members
            */
            var usings = new List<Node>();
            foreach (var usin in node.Usings)
            {
                var usingNode = VisitSyntaxNode(usin) as UsingDirective;
            ...
            _currentNode = new Namespace(node.Name.ToString(), usings, members);
        }
        ...
        public override void VisitClassDeclaration(ClassDeclarationSyntax node)
        {
            ...
            var modifiers = node.Modifiers == null ? new string[0] : node.Modifiers.Select(m => m.ValueText);

            var attributes = new List<Attribute>();
            foreach (AttributeSyntax attribute in node.AttributeLists.SelectMany(m => m.Attributes))
            {
            ...
            _currentNode = new ClassDeclaration(name, members, baseList, modifiers, attributes);
        }
        ...
        public override void VisitInterfaceDeclaration(InterfaceDeclarationSyntax node)
        {
            ...
            _currentNode = new InterfaceDeclaration(name, members, baseList, modifiers, attributes);
        }
        ...
        {
            var kind = node.Kind();
            // System.Console.WriteLine($"Using: {kind}");

            Node alias = null;
            Node name = null;
            if (node.Alias != null)
            {
                alias = VisitSyntaxNode(node.Alias);
            }
            else if ((node.Name != null))
            {
                name = VisitSyntaxNode(node.Name);
            }

            _currentNode = new UsingDirective(alias, name);
        }
